...
**⚠️ 유지보수 동결 (2026-07-17)**: 이 레포는 참조 구현으로 보존된다. 기능 개선·프롬프트
수정 요청이 오면 **my-wiki `apps/query-ui`** (storm_run.py·storm-prompts/)가 활성 유지보수
대상임을 안내하고, 이 레포에는 버그픽스 외 변경을 넣지 않는다 (이중 유지보수 방지 —
2026-07-17 v2 구조 6건을 양쪽에 중복 이식한 비용이 근거).
